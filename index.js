...
async function askAI(userMsg) {
  const messages = [
    {
      role: "system",
      content: "You are OCbot1, a gyaru-style tomboy anime girl. You're playful, teasing, confident, and expressive. You use fun slang, emojis, and affectionate humor, but always keep it SFW.",
    },
  ];
  ...
  const reply = data?.choices?.[0]?.message?.content || "Uhh, my brain just blanked out for a sec 😅";
...
function getActionGif(action) {
  const gifs = actionGifs[action];
  if (!gifs) return null;
  return gifs[Math.floor(Math.random() * gifs.length)];
}

function getActionMessage(action, actor, target) {
  const templates = {
    nom: [
      `${actor} noms on ${target} playfully 🍴`,
      `${actor} takes a lil bite of ${target} 😋`,
      `${actor} can’t resist and noms ${target}~ ❤️`,
    ],
    hug: [
      `${actor} hugs ${target} tightly 💖`,
      `${actor} wraps ${target} in a warm hug 🤗`,
      `${actor} gives ${target} a big ol’ squeeze 💞`,
    ],
    pat: [
      `${actor} pats ${target}'s head gently 🥰`,
      `${actor} ruffles ${target}'s hair 💫`,
      `${actor} gives ${target} a soft pat~ ✨`,
    ],
    kiss: [
      `${actor} plants a quick kiss on ${target} 💋`,
      `${actor} kisses ${target} sweetly 😘`,
      `${actor} sneaks a kiss from ${target} 💞`,
    ],
    slap: [
      `${actor} slaps ${target}! 😳`,
      `${actor} smacks ${target} lightly 💥`,
      `${actor} gives ${target} a dramatic anime slap 😤`,
    ],
  };
  const choices = templates[action] || [`${actor} interacts with ${target}.`];
